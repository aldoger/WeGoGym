...
type (
	MembershipRepository interface {
		...
		UpdateById(ctx context.Context, tx *gorm.DB, id string, duration *int, price *int) (models.Membership, error)
	}
...
func (r *membershipRepository) UpdateById(ctx context.Context, tx *gorm.DB, id string, duration *int, price *int) (models.Membership, error) {
	if tx == nil {
		tx = r.db
	}
